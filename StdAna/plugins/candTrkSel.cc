...
class candTrkSel : public edm::one::EDFilter<edm::one::SharedResources> {
public:
  explicit candTrkSel(const edm::ParameterSet&);
  ~candTrkSel() override;
...
//
candTrkSel::candTrkSel(const edm::ParameterSet& iConfig)
...
bool candTrkSel::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
void candTrkSel::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
