...
class isoTrkSel : public edm::one::EDFilter<edm::one::SharedResources> {
public:
  explicit isoTrkSel(const edm::ParameterSet&);
  ~isoTrkSel() override;
...
//
isoTrkSel::isoTrkSel(const edm::ParameterSet& iConfig)
...
bool isoTrkSel::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
void isoTrkSel::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
